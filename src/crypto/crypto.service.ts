...
export class CryptoService {
    ...
    async buyCrypto(userId: string, symbol: string, fromAccountId: string, amount?: number, fiatAmount?: number) {
        this.logger.log(
            `Buy crypto request: user=${userId}, symbol=${symbol}, amount=${amount}, fiatAmount=${fiatAmount} fromAccountId=${fromAccountId}`
        );
    ...
    async sellCrypto(userId: string, symbol: string, toAccountId: string, amount?: number, fiatAmount?: number) {
        this.logger.log(
            `Sell crypto request: user=${userId}, symbol=${symbol}, amount=${amount}, fiatAmount=${fiatAmount}`
        );
    ...
    async swapCrypto(userId: string, fromSymbol: string, toSymbol: string, fromAmount?: number, toAmount?: number) {
    ...
    async getTransactions(userId: string, symbol?: string, type?: string, limit: number = 20, offset: number = 0) {
