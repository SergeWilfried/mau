...
export class StorageService {
    ...
    async uploadFile(bucket: string, path: string, file: Buffer, contentType: string): Promise<string> {
        const { data, error } = await this.supabaseService.getAdminClient().storage.from(bucket).upload(path, file, {
            contentType,
            upsert: false
        });
    ...
    async deleteFile(bucket: string, path: string): Promise<void> {
        const { error } = await this.supabaseService.getAdminClient().storage.from(bucket).remove([path]);
    ...
    async listFiles(bucket: string, folder: string) {
        const { data, error } = await this.supabaseService.getAdminClient().storage.from(bucket).list(folder);
